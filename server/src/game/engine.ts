 ...
 * Most expensive hardware (by fundingRunningCost) goes offline first when balance is insufficient.
 * @param fundingBalance - current funding balance (after adding income for this tick)
 * @param elapsed - tick duration in seconds (used to compute tick cost)
 */
export function resolveActiveHardware(
  ...
  const tickCost = totalRunningCostPerSec * elapsed;

  if (fundingBalance >= tickCost) return active;

  // Sort by cost DESC, take units offline one by one until balance covers tick cost
  ...
  for (const hw of expensiveFirst) {
    while (active[hw.id] > 0 && totalRunningCostPerSec * elapsed > fundingBalance) {
      active[hw.id]--;
...
export function computeRates(state: GameState): {
  ...
  // Pass elapsed=1 so comparison is per-second against current balance
  const activeHardware = resolveActiveHardware(state, state.funding.toNumber(), 1);
...
export function tick(state: GameState, elapsedMs: number): TickResult {
  ...
  // ── Funding: add income, resolve active hardware based on new balance, deduct running cost ──
  const fundingIncome = rates.fundingPerSecond.mul(elapsed);
  const fundingBeforeDeduction = state.funding.add(fundingIncome);

  // Resolve active hardware based on balance (after adding income)
  const activeHardwareForTick = resolveActiveHardware(state, fundingBeforeDeduction.toNumber(), elapsed);

  // Compute running cost for this tick based on actually-active hardware
  let totalRunningCostPerSec = 0;
  for (const hw of HARDWARE) {
    if (hw.fundingRunningCost > 0) {
      totalRunningCostPerSec += activeHardwareForTick[hw.id] * hw.fundingRunningCost;
    }
  }
  const runningCostThisTick = totalRunningCostPerSec * elapsed;
  const funding = Decimal.max(D0, fundingBeforeDeduction.sub(runningCostThisTick));

  // Recompute rates accounting for which hardware is actually online this tick
  // (rates already uses state.funding; for token/compute we use the same rates since
  //  computeRates resolves hardware based on current balance which is now funded)
